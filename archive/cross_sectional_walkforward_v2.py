...
# -*- coding: utf-8 -*-
"""
跨截面Rank + 滚动训练(Walk-forward) + 波动率缩放 V2

修复V1回测逻辑问题：
- 模型仍基于未来5/10/20日收益训练（学习截面排名信号）
- 回测使用次日(1日)收益率计算策略收益，避免N日复利的重复计算偏差
- 每日调仓，计算日收益，复利是合理的
"""
...
def load_data():
    ...
    for symbol in symbols:
        ...
        if os.path.exists(cache_file):
            ...
            df['symbol'] = symbol
            all_data.append(df)
    return pd.concat(all_data, ignore_index=True), symbols
...
def walk_forward(df, feature_cols, train_label_col, test_label_col='next_day_return',
                 train_window=120, retrain_freq=5):
    """
    Walk-forward滚动训练回测
    - train_label_col: 模型训练用的目标变量（如future_return_5d）
    - test_label_col: 回测用的目标变量（固定为next_day_return，避免N日复利偏差）
    """
    ...
    for i in range(train_window, len(dates), retrain_freq):
        ...
        daily = test_df.groupby('trade_date').apply(lambda g: pd.Series({
            'long_return': g.loc[g['daily_rank'] >= 0.8, test_label_col].mean(),
            'short_return': g.loc[g['daily_rank'] <= 0.2, test_label_col].mean(),
        })).reset_index()
...
def plot_results(df, period, path):
    ...
    axes[0,0].set_title(f'累计收益 ({period}日预测模型, 1日持仓)')
...
def main():
    print("=" * 70)
    print("跨截面Rank + 滚动训练 + 波动率缩放 V2")
    print("修复：回测使用1日持仓收益，避免N日复利偏差")
    print("=" * 70)
    ...
    feature_cols = [c for c in factor_df.columns if c not in [
        'trade_date', 'symbol', 'future_return_5d', 'future_return_10d',
    ...
    for period in [5, 10, 20]:
        ...
        print(f"\n{'='*50}\n{period}日预测模型 - Walk-forward (1日持仓回测)\n{'='*50}")
        res = walk_forward(factor_df, feature_cols, train_label,
                           test_label_col='next_day_return',
                           train_window=120, retrain_freq=5)
        if res is None or len(res) == 0:
            print("空结果"); continue
        res = vol_scale(res, target=0.15, lookback=20)
        m_ls = calc_metrics(res['ls_return'])
        m_sc = calc_metrics(res['scaled_return'])
        for name, m in [('纯多空', m_ls), ('波动率缩放', m_sc)]:
            print(f"\n[{name}] 总收益:{m['total']*100:.1f}% 年化:{m['annual']*100:.1f}% "
                  f"夏普:{m['sharpe']:.2f} 回撤:{m['max_dd']*100:.1f}% "
                  f"Calmar:{m['calmar']:.2f} 胜率:{m['win']*100:.1f}% 天数:{m['n']}")
        plot_results(res, period, os.path.join(OUTPUT_DIR, f'cs_walkforward_v2_{period}d.png'))
