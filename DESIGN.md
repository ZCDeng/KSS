...
- Investment daily V1: the compiled HTML is an editorial sheet, not a process dump. Lead with a masthead and four metric cards, then 摘要 / 正文 / 精判卡 / 注释. Formula ids and evidence hashes stay in footnotes. Desktop preview must carry `<head>` CSS into the Kami shell and must not restyle `.kss-report`.
