...
# -*- coding: utf-8 -*-
"""
688322 单股深度分析：因子重要性 + 最优决策指标组合 + 10日预测
...
def generate_factors(df):
    ...
    if 'pe' in df.columns:
        ...
        if df['pe'].notna().sum() / len(df) > 0.5:
            ...
            print(f"    警告: PE缺失率{(1-df['pe'].notna().sum()/len(df))*100:.0f}%，跳过PE因子")
...
def plot_importance(imp_df, path):
    ...
    ax.set_title('688322 LightGBM 特征重要性 Top20')
    plt.tight_layout()
...
def main():
    print("=" * 70)
    print("688322 单股深度分析：因子重要性 + 最优决策指标组合 + 10日预测")
    print("=" * 70)
    ...
    # 保存特征重要性图
    plot_importance(imp_10d, os.path.join(OUTPUT_DIR, '688322_importance_10d.png'))
    ...
    plot_equity_curves(factors, top_strategies, 
                       os.path.join(OUTPUT_DIR, '688322_strategy_compare.png'))
    ...
    print("\n【推荐决策指标组合】")
    print("  基于LightGBM特征重要性和回测结果，688322最有效的决策指标为:")
