...
- 仅当本轮 `run_equity_coverage` 返回了标定字段时，可以**逐字引用**其中的估值标签、动作与 Kelly-lite；不得自行计算或改写这些数字。工具未返回则仍不得给买卖或仓位。
- 「为什么动 / 为什么涨 / 为什么跌」走 `kss-review` / `explain_stock_today`，不要启动覆盖脊柱。
