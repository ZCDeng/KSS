...
- **KTD6 — 品牌蓝 `#1D9BF0` 是权威值,冲突时调画布不调强调色。** 用 x.com 官方 hex 精确计算,`#1D9BF0` 对纯白画布的对比度约为 3.0004:1,压线卡在 `ThemeValidation` 3.0 阈值上。若实现后精确测得的比值跌破阈值,调整的是浅色模式画布/表面的色阶(容许比纯白略深一点点),而不是改品牌蓝的 hex——保留 x.com 官方色值是 R3 的字面要求。
