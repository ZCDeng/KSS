...
- 两张新表要不要进自由 SQL 查询的可见表清单。让它可见需要先解决一个语义问题，现有的登记门要求新旧真源逐行核对通过，全新建的表没有旧真源可对，直接标通过等于伪造一次核对；要么给这个门扩一类新建场景，要么两张表不进清单、只靠 U8 的专用工具供数，代价是任何没预想到的交叉提问答不了。这条决定 U8 是否改 `kss/storage/duck.py`。
- 应用内 AI 面板要不要也拿 U8 的两个只读工具。紫苏叶富化的先例是给 MCP 不给面板，并有一条测试把这个不对称钉住；这层数据是用户自己维护的判断，「这只票我标了吗」天然会问面板，性质与紫苏叶不同。加一条工具声明的成本接近零，但要作为一个决定写下来而不是照抄先例。
...
- 是否把「自选列表全部个股的 8 问答完」设为首版准入门槛。不设的话，区位这一维度在四个落点大概率长期停在「未尽调」，等于只上线了色点那一半。
