...
**已交付**
1. **往期跟踪可点开看当日选股**：往期跟踪从纯表格改成可展开的日卡 `TrackingDayCard`（chevron 展开），点开露出该预测日逐只选股的 1d/5d/20d 实际收益，每只仍可点进个股详情。
2. **修自选/股票池死空列**：`StockBrowserView` 也是嵌套 `NavigationSplitView`（中间死空列，截图红框）。改成 `HStack(列表 300pt + 自带搜索框 | Divider | 详情 fill)`，空隙消除、K 线图变宽。原 `.searchable` 换成列内手写搜索框。
3. **总览重新设计**：从 6 段全宽竖排改成清晰分组——KPI 概览条 → 两栏「今日推荐(紧凑编号列表) | 纸交易跟踪卡(年化/Sharpe/回撤/胜率/样本)」→ 北证50扫描 → 两栏「最近复盘 | 回测证据」。大卡片换紧凑列表，信息密度与层次更清楚。

**验证**
- `swift build`、`build_and_run --verify` 通过。
- 实机：自选页详情紧贴列表无死空列、图更宽；往期跟踪点 2026-05-11 展开 10 只选股逐只 1d/5d/20d（华大智造 -3.91/-2.04/-11.40，奥比中光-W -2.33/+7.81/+54.46）；总览两栏布局清晰。
...
5. **北证日线（追加）**：`_bj_detail`/`_bj_summary` 改读 `scan_bj50` 已落地的 Tushare 日线缓存 `storage/bj_cache/<code>_daily.csv`（零联网），BJ 个股补齐 K 线 + MA5/MA20 + 量能 + 分析指标；池内 BJ 行也带上涨跌幅/均线。缓存覆盖至 2026-06-05（科创到 06-18）。
6. **刷新北证日线任务**：新增 `scripts/refresh_bj_daily.py`（复用 `TushareClient`，按各股缓存末日增量拉 `pro.daily` 到今天，保持原 ISO 列格式回写），绕开 scan_bj50 里硬编码的 `end_date=20260607`；bridge 加 `refresh-bj-daily` 任务（正式 lane）、`KSSTask.refreshBjDaily`，Runbook「正式」组出现「刷新北证日线」按钮。sandbox 无网时优雅返回「0/50 新增」，真机联网拉增量。
